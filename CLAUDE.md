...
- `config.py`: Frozen `Settings` dataclass populated from env vars plus the repo-root `sources.json` catalog by `get_settings()` (LRU-cached). Non-source settings use the `ARTICLE_WRITER_` prefix; source enable flags, queries, feeds, keywords, and weights live in `sources.json`.
...
Copy `.env.example` to `.env` for runtime settings, and edit `sources.json` for source enable flags, queries, feeds, keywords, and weights. `get_settings()` is cached for the process lifetime — restart to pick up env or source catalog changes.
